...
use crate::models::specification::{
    FormalSpecification, Specification, SpecificationOptions, SpecificationTranslation, 
    ValidationReport, ValidationIssue, VerificationTemplate, IssueSeverity
};
...
impl From<SpecGenError> for AxiomError {
    fn from(err: SpecGenError) -> Self {
        match err {
            SpecGenError::ApiError(msg) => AxiomError::ExternalToolError { 
            ...
            SpecGenError::NetworkError(msg) => AxiomError::ExternalToolError { 
            ...
            SpecGenError::HttpError { status, message } => AxiomError::ExternalToolError { 
                tool: "HTTP".to_string(), 
...
impl LLMSpecificationGenerator {
    ...
    pub fn save_to_project(&self, project_name: &str, spec: &Specification) -> AxiomResult<PathBuf> {
        ...
        fs::create_dir_all(&project_dir)
            .map_err(|e| AxiomError::SystemError(format!("Failed to create project directory: {}", e)))?;
        ...
        let mut spec_file = File::create(&spec_file_path)
            .map_err(|e| AxiomError::SystemError(format!("Failed to create specification file: {}", e)))?;
            
        spec_file.write_all(spec.formal_spec.spec_code.as_bytes())
            .map_err(|e| AxiomError::SystemError(format!("Failed to write specification file: {}", e)))?;
        ...
        let mut metadata_file = File::create(&metadata_file_path)
            .map_err(|e| AxiomError::SystemError(format!("Failed to create metadata file: {}", e)))?;
        ...
        let mut requirements_file = File::create(&requirements_file_path)
            .map_err(|e| AxiomError::SystemError(format!("Failed to create requirements file: {}", e)))?;
            
        requirements_file.write_all(requirements_str.as_bytes())
            .map_err(|e| AxiomError::SystemError(format!("Failed to write requirements file: {}", e)))?;
    ...
    fn get_domain_context(&self, domain: &Domain) -> String {
        ...
        if let Some(context) = self.domain_contexts.get(domain_name) {
            format!(
                ...
                domain_name,
                context.description,
                context.common_properties.iter()
                    .map(|p| format!("- {}", p))
                    .collect::<Vec<_>>()
                    .join("\n"),
    ...
    fn render_template(&self, template_name: &str, params: &HashMap<String, String>) -> Result<String, SpecGenError> {
        let template = self.config.get_template(template_name)
            .ok_or_else(|| SpecGenError::TemplateError(format!("Template not found: {}", template_name)))?;
    ...
    async fn call_llm_api(&self, prompt: &str) -> Result<String, SpecGenError> {
        ...
        let (api_endpoint, model) = match provider.as_str() {
            "openai" => {
                ...
                (
                    self.config.llm_api.api_endpoint.clone()
                        .unwrap_or_else(|| "https://api.openai.com/v1/chat/completions".to_string()),
                    self.config.llm_api.model.clone()
        ...
        if provider == "anthropic" {
            ...
            debug!("Claude request: {}", serde_json::to_string(&claude_request).unwrap_or_default());
            
            debug!("Sending request to Anthropic API");
            debug!("Anthropic API endpoint: {}", api_endpoint);
            
            let request_builder = self.http_client.post(&api_endpoint)
            ...
            debug!("Anthropic response structure: {}", 
                   serde_json::to_string_pretty(&response_json).unwrap_or_else(|_| "unable to format".to_string()));
            ...
            let content = if let Some(content_array) = response_json["content"].as_array() {
                if let Some(first_content) = content_array.get(0) {
                    if let Some(text) = first_content["text"].as_str() {
                        ...
                        return Err(SpecGenError::ParseError("Missing text in Anthropic response content".to_string()));
                    }
                } else {
                    warn!("No content items in Anthropic response");
                    return Err(SpecGenError::ParseError("Empty content array in Anthropic response".to_string()));
                }
            } else {
                ...
                response_json["content"].as_str()
                    ...
                    .ok_or_else(|| {
                        ...
                        SpecGenError::ParseError("Unable to find content in Anthropic response".to_string())
                    })?.to_string()
            };
            ...
            debug!("Calling LLM API with prompt: {} (truncated)", 
            ...
            debug!("OpenAI response structure: {}", 
                  serde_json::to_string_pretty(&response_json_value).unwrap_or_else(|_| "unable to format".to_string()));
            ...
            let response_json: ChatResponse = match serde_json::from_str(&response_text) {
                ...
                Err(e) => {
                    ...
                    return Err(SpecGenError::ParseError(format!("Failed to parse API response: {}", e)));
                }
            };
    ...
    for line in content.lines() {
        ...
        for pattern in &patterns {
            if trimmed.starts_with(pattern) {
                let dep = trimmed[pattern.len()..].trim()
                    .split_whitespace()
                    .next()
                    .unwrap_or("")
...
impl SpecificationGenerator for LLMSpecificationGenerator {
    async fn generate_specification(
        ...
        params.insert("verification_language".to_string(), options.verification_language.to_string());
        ...
        // Render the template
        let prompt = self.render_template(template_name, &params)
        ...
        let formal_spec = self.parse_formal_specification(&response, options.verification_language.clone())
            .map_err(AxiomError::from)?;
        ...
        let spec = Specification {
            ...
            metadata: crate::models::specification::SpecificationMetadata {
                ...
                verification_system: match options.verification_language {
                    ...
                    _ => crate::models::common::VerificationSystem::Custom(
    ...
    async fn refine_specification(
        ...
        let mut params = HashMap::new();
        params.insert("verification_language".to_string(), spec.formal_spec.verification_language.to_string());
        ...
        let formal_spec = self.parse_formal_specification(
            &response, 
            spec.formal_spec.verification_language.clone()
        ).map_err(AxiomError::from)?;
    ...
    async fn validate_specification(
        ...
        info!("Validating specification with depth: {}", 
            match validation_depth {
        ...
        // For simplicity, we only implement basic validation
        // This is enough to check if the specification looks valid at a syntax level
        info!("Simplified validation: only performing basic validation");
        self.validate_syntax(spec).await
    }
    ...
    async fn translate_to_properties(
        ...
        for section in sections {
            ...
            let requirement = lines[0].strip_prefix("Requirement: ")
                .unwrap_or(lines[0])
                .to_string();
                
            let interpreted_property = lines[1].strip_prefix("Formal property: ")
                .unwrap_or(lines[1])
                .to_string();
                
            let formal_representation = lines[2].strip_prefix("Mathematical form: ")
                .unwrap_or(lines[2])
                .to_string();
                
            let confidence_str = lines[3].strip_prefix("Confidence: ")
                .unwrap_or("0.7")
                .trim();
    ...
    async fn convert_to_formal_specification(
        ...
        let properties_text = translations.iter()
            .map(|t| format!(
                "Requirement: {}\nFormal Property: {}\nFormalization: {}",
                t.requirement,
        ...
        // Parse the response into a formal specification
        let formal_spec = self.parse_formal_specification(&response, target_language)
            .map_err(AxiomError::from)?;
    ...
    async fn translate_specification(
        ...
        info!("Translating specification from {} to {}", 
            spec.formal_spec.verification_language, target_language);
        ...
        // Parse the response into a formal specification
        let formal_spec = self.parse_formal_specification(&response, target_language.clone())
            .map_err(AxiomError::from)?;
        ...
        let translated_spec = Specification {
            ...
            metadata: crate::models::specification::SpecificationMetadata {
                ...
                verification_system: match target_language {
                    ...
                    VerificationLanguage::Why3Lang => crate::models::common::VerificationSystem::Why3,
                    VerificationLanguage::Z3SMT => crate::models::common::VerificationSystem::Z3,
                    _ => crate::models::common::VerificationSystem::Custom(target_language.to_string()),
                },
                domain: spec.metadata.domain.clone(),
    ...
    async fn verify_specification_completeness(
        ...
        let is_complete = response.to_lowercase().contains("the specification is complete: true") ||
    ...
    async fn generate_verification_code(
        ...
        let target_language = match target_system {
            ...
            crate::models::common::VerificationSystem::Isabelle => VerificationLanguage::IsabelleLang,
        ...
        let specification_code = if spec.formal_spec.verification_language == target_language {
            ...
            let translated_spec = self.translate_specification(spec, target_language.clone())
                .await?;
        ...
        for (i, section) in sections.iter().enumerate() {
            ...
            let template_code = match content.find("```") {
                Some(start_idx) => {
                    ...
                    let code_end = content[code_start..].find("```").map(|e| code_start + e).unwrap_or(content.len());
    ...
    async fn apply_template(
        ...
        // Prepare the prompt
        let properties_text = properties.iter()
            .map(|p| format!(
                "Property {}: {} - {}",
                p.id,
                p.description,
        ...
        // Parse the response into a formal specification
        let formal_spec = self.parse_formal_specification(&response, template.language.clone())
            .map_err(AxiomError::from)?;
    ...
    async fn export_specification(
        ...
        if let Some(parent) = output_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| AxiomError::SystemError(format!("Failed to create directory: {}", e)))?;
        }
        
        // Write the specification to the file
        std::fs::write(output_path, &spec.formal_spec.spec_code)
    ...
    async fn import_specification(
        ...
        let spec_code = std::fs::read_to_string(spec_file)
        ...
        let formal_spec = self.parse_formal_specification(&spec_code, language.clone())
            .map_err(AxiomError::from)?;
...
impl LLMSpecificationGenerator {
    async fn validate_syntax(&self, spec: &Specification) -> AxiomResult<ValidationReport> {
        ...
        let is_valid = response.to_lowercase().contains("the specification syntax is valid: true") ||
            ...
            if line.starts_with("Line ") || line.starts_with("Location ") {
                ...
                if let Some(start) = line.find("Line ") {
                    if let Some(end) = line[start + 5..].find(':') {
                        if let Ok(line_num) = line[start + 5..start + 5 + end].trim().parse::<usize>() {
                ...
                current_suggestion = Some(line.trim_start_matches("Suggestion:").trim().to_string());
            }
        }
    ...
    async fn validate_type_checking(&self, spec: &Specification) -> AxiomResult<ValidationReport> {
        ...
        // Parse the response to determine validity and extract issues
        let is_valid = response.to_lowercase().contains("the specification passes type checking: true") ||
    ...
    async fn validate_formal_verification(&self, spec: &Specification) -> AxiomResult<ValidationReport> {
        ...
        // Parse the response to determine validity and extract issues
        let is_valid = response.to_lowercase().contains("the specification can be formally verified: true") ||
                       response.to_lowercase().contains("can the specification be formally verified as written? true");
